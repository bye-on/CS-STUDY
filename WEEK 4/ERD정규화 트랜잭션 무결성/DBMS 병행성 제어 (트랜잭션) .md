...
| 기능                                   | 설명                                                            | 관련 메커니즘                                       |
| -------------------------------------- | --------------------------------------------------------------- | --------------------------------------------------- |
| **동시성 제어 (Concurrency Control)**  | 여러 트랜잭션이 동시에 같은 데이터를 다룰 때 충돌하지 않게 함   | **Locking (2PL)**, **Timestamp Ordering**, **MVCC** |
| **직렬 가능성 보장 (Serializability)** | 인터리빙된 실행이라도 순차 실행과 같은 결과 보장                | 충돌 직렬 가능성 검사                               |
| **격리 수준 조정 (Isolation Level)**   | READ UNCOMMITTED, READ COMMITTED, REPEATABLE READ, SERIALIZABLE | 트랜잭션 간 간섭 정도 조절                          |
| **교착상태 관리 (Deadlock Handling)**  | 서로 기다리는 트랜잭션이 있을 때 한쪽을 중단시켜 복구           | Wait-for Graph 탐지, Timeout                        |
...
| 이름                      | 형태                  | 의미                                    | 대표적인 이상 현상          |
| ------------------------- | --------------------- | --------------------------------------- | --------------------------- |
| **WR 충돌 (Write–Read)**  | T1이 쓰고 → T2가 읽음 | T2가 “아직 커밋되지 않은” 데이터를 읽음 | **Dirty Read (오손 판독)**  |
| **RW 충돌 (Read–Write)**  | T1이 읽고 → T2가 씀   | T1이 읽은 데이터를 T2가 나중에 수정함   | **Non-repeatable Read**     |
| **WW 충돌 (Write–Write)** | T1이 쓰고 → T2가 씀   | 두 트랜잭션이 같은 데이터를 덮어씀      | **Lost Update (갱신 손실)** |
